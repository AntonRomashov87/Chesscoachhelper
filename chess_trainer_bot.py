"""
♟️ Chess Trainer Bot v3.2 — Telegram-бот для тренера шахової школи
ВИПРАВЛЕНО: MongoDB ініціалізується в main() після завантаження змінних середовища
...
# ─────────────────────────────────────────────
# MONGODB — глобальні змінні, ініціалізуються в main()
# ─────────────────────────────────────────────
mongo_client = None
mdb = None

def init_mongo():
    """Викликається в main() — коли Railway вже передав всі змінні середовища"""
        ...
        if 115 <= diff <= 125:
            ...
            for pid in db_get_parents():
                try:
                    ...
                    pass
            if sent:
                logger.info(f"Нагадування надіслано {sent} батькам")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Тут ви можете переглядати розклад, домашні завдання та відвідуваність.\n\n"
...
async def parent_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📅 Розклад занять":
        ...
        if not schedule:
            ...
            msg = "📅 Розклад занять:\n\n" + "".join(f"📌 {s['day']} {s['time']} — {s['group']} ({s['place']})\n" for s in sorted_s)
        ...
        if not student_name:
            await update.message.reply_text(
                "⚠️ Ваша дитина ще не прив'язана.\nЗверніться до тренера.",
                reply_markup=parent_keyboard()
            )
            return PARENT_MENU
...
async def students_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "⬅️ Головне меню":
        ...
        if not students:
            ...
            msg = "📋 Список учнів:\n\n" + "".join(f"{i}. {s['name']} — {s['level']} | {s['phone']}\n" for i, s in enumerate(students, 1))
...
async def add_student(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        student = {"name": parts[0], "level": parts[1], "phone": parts[2], "added": datetime.now().strftime("%d.%m.%Y")}
        db_add_student(student)
        logger.info(f"[ADD_STUDENT] ✅ Збережено: {student['name']}")
        await update.message.reply_text(f"✅ Учня {student['name']} успішно додано!", reply_markup=students_keyboard())
...
async def add_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text
    logger.info(f"[ADD_SCHEDULE] Отримано: '{text}'")
    if text == "⬅️ Головне меню":
        await update.message.reply_text("Головне меню:", reply_markup=main_keyboard())
        return MAIN_MENU
    ...
    try:
        parts = [p.strip() for p in text.split("|")]
        logger.info(f"[ADD_SCHEDULE] parts: {parts}")
        if len(parts) < 4:
        ...
        logger.info(f"[ADD_SCHEDULE] ✅ Збережено: {entry}")
        await update.message.reply_text(
            f"✅ Заняття {entry['day']} {entry['time']} додано!\n🔔 Батьки отримають нагадування за 2 год.",
            reply_markup=schedule_keyboard()
        )
    except Exception as e:
        logger.error(f"[ADD_SCHEDULE] ❌ {e}")
        await update.message.reply_text(
        ...
        for pid in db_get_parents():
            try:
                await context.bot.send_message(chat_id=int(pid),
                    text=f"📚 Нове домашнє завдання!\n\n👥 Група: {hw['group']}\n📝 Завдання: {hw['task']}\n📅 Здати до: {hw['deadline']}")
                sent += 1
            except Exception:
                pass
        await update.message.reply_text(f"✅ Завдання додано! Надіслано {sent} батькам.", reply_markup=homework_keyboard())
    except Exception as e:
        logger.error(f"[ADD_HOMEWORK] ❌ {e}")
        await update.message.reply_text(
...
async def news_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "⬅️ Головне меню":
        ...
        await update.message.reply_text(
            "Введіть новину у форматі:\n<b>Заголовок | Текст</b>\n\nПриклад: Турнір у квітні | Запрошуємо 12 квітня!",
...
async def add_news(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        parts = [p.strip() for p in text.split("|")]
        if len(parts) < 2:
            raise ValueError(f"Потрібно 2 поля через |, отримано {len(parts)}")
        ...
        for pid in db_get_parents():
            try:
                ...
                pass
        await update.message.reply_text(f"✅ Новину опубліковано! Надіслано {sent} батькам.", reply_markup=news_keyboard())
    except Exception as e:
    ...
    if text == "⬅️ Головне меню":
        ...
        if not parents:
            ...
            for pid, info in parents.items():
                msg += f"• {info['name']} → учень: {info.get('student','не вказано')} (ID: {pid})\n"
        ...
        if not students:
            ...
            return CHAT_MENU
        keyboard = [[InlineKeyboardButton(f"{info['name']} → {info.get('student','не вказано')}", callback_data=f"link_parent_{pid}")] for pid, info in parents.items()]
...
async def attendance_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "⬅️ Головне меню":
        ...
        keyboard = [[InlineKeyboardButton(f"✅ {s['name']}", callback_data=f"att_present_{i}"),
                     InlineKeyboardButton(f"❌ {s['name']}", callback_data=f"att_absent_{i}")] for i, s in enumerate(students)]
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("link_parent_"):
        ...
        await query.edit_message_text(f"👤 Батько: <b>{parent_name}</b>\n\nОберіть учня:", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(keyboard))
...
# ─────────────────────────────────────────────
def main():
    # ✅ MongoDB ініціалізується ТУТ — після того як Railway передав змінні середовища
    try:
        init_mongo()
    except Exception as e:
    ...
    print("♟️ Chess Trainer Bot v3.2 + MongoDB Atlas запущено!")
